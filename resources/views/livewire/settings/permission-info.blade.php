    ...
    <div class="flex">
        <div class="relative inline-block text-left w-full pr-4 lg:pr-0 md:pr-0">
            <div id="ComponentSelector" class="w-full flex items-center">
                ...
                <div class="selectWrapperLG w-full">
                    <select id="Resources" class="open-sans-soft-regular border-r-1 border-t-1 border-b-1 border-gray-300 border-solid bg-[#707070] text-white text-lg hover:bg-[#4a4a4a] w-full md:p-6 lg:p-6 p-6 pr-10 rounded-r-lg font-bold shadow-md" wire:change="$js.ChangeResource($event,$event.target.value)">
                        @foreach ($Resources as $TempResource)
                            @if (isset($Resource))
                                @if($TempResource->resource_name == $Resource)
                                    <option value="{{ $TempResource->resource_name  }}" selected >{{ $TempResource->resource_name  }}</option>
                                @else
                                    <option value="{{ $TempResource->resource_name  }}">{{ $TempResource->resource_name  }}</option>
                                @endif
                            @endif
                        @endforeach
                    </select>
    ...
    <div id="AddMenu" class="hide relative transition-all duration-100">
        ...
        <form>
            <div id="AddPermission" class="pt-24 pb-10 relative bg-[#00719d] z-1 pl-10 pt-1 pr-3 mt-2 text-white h-[640px] rounded-lg w-[400px] overflow-x-visible overflow-y-scroll">
                @if (isset($resourcesInfo))
                    <livewire:components.frm-select-box selectMessage="Resource Subname:" id="resourceSubName" :options="$resourcesInfo" optionName="resource_sub_name" optionId="resource_sub_name"></livewire:components.frm-select-box>
                @endif

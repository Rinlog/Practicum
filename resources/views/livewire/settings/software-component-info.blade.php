<div id="MainWindowSettings" class="flex flex-col lg:flex-row lg:w-[1750px] gap-10">
    ...
    <div class="lg:p-10 md:p-10 pr-10 pl-2 pt-2 bg-white shadow-md mt-8 rounded-lg h-screen md:h-[645px] lg:h-[645px]">
        <span class="flex items-center justify-between mb-4">
            ...
            <span class="flex gap-4 items-center">
                <label class="text-[#1c648c] font-semibold text-3xl">Component Information</label>
    ...
    <div id="AddMenu" class="hide relative transition-all duration-100">
        ...
        {{-- form --}}  
        <form>
            <div id="AddComponent" class="pt-24 pb-10 relative bg-[#00719d] z-1 pl-10 pt-1 pr-3 mt-2 text-white h-[640px] rounded-lg w-[400px] overflow-x-visible overflow-y-scroll">
                    <livewire:components.req-underline-input id="ComponentName" placeholder="Component Name" type="text"></livewire:components.req-underline-input>
                    <livewire:components.underline-input id="description" placeholder="Description" type="text"></livewire:components.underline-input>
                    <livewire:components.form-chkbox id="hasAPI" text="Has API"></livewire:components.form-chkbox>
            </div>
            {{-- Confirm Section --}}
    ...
    <div id="EditMenu" class="hide relative transition-all duration-100">
        ...
        {{-- form --}}  
        <form>
            <div id="EditComponent" class="pt-24 pb-30 relative bg-[#00719d] z-1 pl-10 pt-1 pr-3 mt-2 text-white h-[640px] rounded-lg w-[400px] overflow-x-visible overflow-y-scroll">
                    <livewire:components.req-underline-input id="ComponentName" placeholder="Component Name" type="text"></livewire:components.req-underline-input>
                    <livewire:components.underline-input id="description" placeholder="Description" type="text"></livewire:components.underline-input>
                    <livewire:components.form-chkbox id="hasAPI" text="Has API"></livewire:components.form-chkbox>
            </div>
    ...
    <livewire:alert.notification key="{{ Str::random() }}"></livewire:alert.notification>
        ...
        <script>
            ...
            $js("AddConfirm",function(e){
                if ($("#AddComponent #ComponentName").val() == ""){
                    return;
                }
                e.preventDefault();
                let FormVals = PopulateArrayWithVals("AddComponent");
                ...
                //adding Component id placeholder
                let ComponentID = document.createElement("td");
                ComponentID.textContent = "Will generate automatically";
                ...
                $("#InfoTable").append(tr);
                setAlertText("Successfully added Component");
                displayAlert();
                closeAddMenu()
            });
            function PopulateArrayWithVals(EditAdd){
                let FormVals = [];
                FormVals.push($(`#${EditAdd} #ComponentName`).val());
                FormVals.push($(`#${EditAdd} #hasAPI`).prop("checked"));
            ...
            $js("EditConfirm",function(e){
                if ($("#EditComponent #ComponentName").val() == ""){
                    return;
                }
                e.preventDefault();
                let FormVals = PopulateArrayWithVals("EditComponent");
                let OGCopy = $("#"+SpaceToUnderScore(EditItem)).clone(false);
                $("#"+SpaceToUnderScore(EditItem)).children().each(function(index){
                    //we exclude the checkbox, sequence num, Component ID
                    if (index >=3){
                ...
                if (Result != ""){
                    ...
                    setAlertText("Successfully updated Component");
                    displayAlert();
                    closeEditMenu();
                }
            });
            ...
            $js("CloseOpenEdit",function(){
                ...
                if (EditMenuStatus == false){
                    ...
                    $("#EditComponent #ComponentName").val(Obj["SOFTWARE COMPONENT NAME"]);
                    $("#EditComponent #hasAPI").val(Obj["HAS API"]);
                    $("#EditComponent #description").val(Obj["DESCRIPTION"]);
                }
                else{
                    closeEditMenu();
                }
            ...
            async function refresh(){
                ...
                $("#ConfirmDelete").click(function(e){
                    ...
                    setAlertText("Successfully deleted Component(s)");
                    displayAlert();
                });
                closeAddMenu();
                closeEditMenu();
                    ...
                    try{
                        if (Type.includes("UPDATE")){
                            if (Result[index] == 0){
                                ...
                                ErrorMsg += "Failed to update Component \"" + Obj["Component NAME"] + "\"<br>";
                            }
                        }
                            ...
                            if (Result[index] != true){
                                Errors = true;
                                let Obj = JSON.parse(ItemInfo);
                                ErrorMsg += "Failed to insert Component \"" + Obj["Component NAME"] + "\"<br>";
                            }
                        }
                        else if (Type.includes("DELETE")){
                            if (Result[index] == 0){
                                Errors = true;
                                ErrorMsg += "Failed to delete Component(s) " + ItemInfo + "<br>";
                            }
                        }
                    }
                    catch(ex){
            ...
            $js("DownloadCSV",async function(){
                if (TableObjects.length != 0){
                    let result = exportToCsv("ComponentInfo.csv",TableObjects);

    ...
    <div class="lg:p-10 md:p-10 pr-10 pl-2 pt-2 bg-white shadow-md mt-8 rounded-lg h-screen md:h-[645px] lg:h-[645px]">
        <span class="flex items-center justify-between mb-4">
            ...
            <span class="flex gap-4 items-center">
                <label class="text-[#1c648c] font-semibold text-3xl">User Information </label>
    ...
    <livewire:alert.notification key="{{ Str::random() }}"></livewire:alert.notification>
        ...
        <script>
            ...
            $js("AddConfirm",function(e){
                ...
                e.preventDefault();
                let FormVals = PopulateArrayWithVals("AddUser");
                ...
                $("#InfoTable").append(tr);
                setAlertText("Successfully added User");
                displayAlert();
                closeAddMenu()
            });
            ...
            function PopulateArrayWithVals(EditAdd){
                let FormVals = [];
                FormVals.push($(`#${EditAdd} #userName`).val());
            ...
            $js("EditConfirm",function(e){
                ...
                e.preventDefault();
                let FormVals = PopulateArrayWithVals("EditUser");
                ...
                if (Result != ""){
                    ...
                    ActionsDone.push("UPDATE["+EditItem+"]~!~"+JSON.stringify(TRToObject($("#"+SpaceToUnderScore(EditItem))))+"~!~"+$("#resetPass").prop("checked"));
            ...
            $js("CloseOpenEdit",function(){
                ...
                if (EditMenuStatus == false){
                    ...
                    $("#EditUser #userName").val(Obj["USER NAME"]);
                ...
                ActionsDone.forEach(function(item,index){
                    ...
                    try{
                        if (Type.includes("UPDATE")){
                            ...
                            if (Result[index] == 0){
                                Errors = true;
                                ErrorMsg += "Failed to delete User(s) " + ItemInfo + "<br>";
                            }
                        }
                    }
                    catch(ex){
            ...
            $js("DownloadCSV",async function(){
                if (TableObjects.length != 0){
                    let result = exportToCsv("UserInfo.csv",TableObjects);

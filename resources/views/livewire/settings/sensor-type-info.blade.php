<div id="MainWindowSettings" class="flex flex-col lg:flex-row lg:w-[1750px] gap-10">
    ...
    <div class="lg:p-10 md:p-10 pb-15 pr-10 pl-2 pt-2 bg-white shadow-md mt-8 rounded-lg h-[645px]">
        <span class="flex items-center justify-between mb-4">
            ...
            <span class="flex gap-4 items-center">
                <label class="text-[#1c648c] font-semibold text-3xl">Sensor Type Information</label>
    ...
    <div id="AddMenu" class="hide relative transition-all duration-100">
        ...
        {{-- form --}}  
        <form>
            <div id="AddSensorType" class="pt-24 pb-10 relative bg-[#00719d] z-1 pl-10 pt-1 pr-3 mt-2 text-white h-[640px] rounded-lg w-[400px] overflow-x-visible overflow-y-scroll">
    ...
    <livewire:alert.notification key="{{ Str::random() }}"></livewire:alert.notification>
        ...
        <script>
            ...
            //select box stuff 
            $js('SensorTypeChecked',function(e,id){
            ...
            $js("AddConfirm",function(e){
                if ($("#AddSensorType #SensorTypeName").val() == ""){
                    return;
                }
                e.preventDefault();
                let FormVals = PopulateArrayWithVals("AddSensorType");
                ...
                let SensorTypeID = document.createElement("td");
                SensorTypeID.textContent = "Will generate automatically";
                tr.appendChild(SensorTypeID);
            ...
            function PopulateArrayWithVals(EditAdd){
                let FormVals = [];
                FormVals.push($(`#${EditAdd} #SensorTypeName`).val());
                    ...
                    try{
                        if (Type.includes("UPDATE")){
                            if (Result[index] == 0){
                                ...
                                ErrorMsg += "Failed to update SensorType \"" + Obj["SENSOR TYPE"] + "\"<br>";
                            }
                        }
                            ...
                            if (Result[index] != true){
                                Errors = true;
                                let Obj = JSON.parse(ItemInfo);
                                ErrorMsg += "Failed to insert SensorType \"" + Obj["SENSOR TYPE"] + "\"<br>";
                            }
                        }
                        else if (Type.includes("DELETE")){
                            if (Result[index] == 0){
                                Errors = true;
                                ErrorMsg += "Failed to delete SensorType(s) " + ItemInfo + "<br>";
                            }
                        }
                    }
                    catch(ex){
            ...
            $js("DownloadCSV",async function(){
                if (TableObjects.length != 0){
                    let result = exportToCsv("SensorTypeInfo.csv",TableObjects);
